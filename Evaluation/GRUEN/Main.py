...
""" Scores Calculation """
def get_lm_score(sentences, tokenizer_lm: Optional[Any] = None, model_lm: Optional[Any] = None):
    ...
    if model_lm is None:
        ...
        model = model_lm
    tokenizer = BertTokenizer.from_pretrained(model_name) if tokenizer_lm is None else tokenizer_lm
    lm_score = []
...
def get_cola_score(sentences, cola_config: Optional[Any] = None, cola_tokenizer: Optional[Any] = None,
    ...
    def evaluate_cola(model, candidates, tokenizer, model_name):
        ...
        def load_and_cache_examples(candidates, tokenizer):
            ...
            all_token_type_ids = torch.tensor([[0]*max_length for f in features], dtype=torch.long)
...
def get_grammaticality_score(processed_candidates, tokenizer_lm: Optional[Any] = None, model_lm: Optional[Any] = None,
              cola_config: Optional[Any] = None, cola_tokenizer: Optional[Any] = None,
              cola_model: Optional[Any] = None):
    lm_score = get_lm_score(processed_candidates, tokenizer_lm, model_lm)
    cola_score = get_cola_score(processed_candidates, cola_config, cola_tokenizer, cola_model)
...
def get_gruen(candidates, tokenizer_lm: Optional[Any] = None, model_lm: Optional[Any] = None,
              cola_config: Optional[Any] = None, cola_tokenizer: Optional[Any] = None,
              cola_model: Optional[Any] = None):
    processed_candidates = preprocess_candidates(candidates)
    grammaticality_score = get_grammaticality_score(processed_candidates, tokenizer_lm, model_lm, cola_config,
                                                    cola_tokenizer, cola_model)
